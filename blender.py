...
class TOOL_OT_3dp_initialize(Operator):
    ...
    @classmethod
    def poll(cls, context):
        return context.active_object.mode == "OBJECT" and len(context.selected_objects) == 1
...
class TOOL_OT_3dp_dissolve(Operator):
    ...
    @classmethod
    def poll(cls, context):
        return context.active_object.mode == "OBJECT" and len(context.selected_objects) > 0
...
class VIEW3D_PT_3dpkbd_dissolve(Panel):
    ...
    def draw(self, context):
        ...
        row = layout.row()
        row.active = context.active_object.mode == "OBJECT" and len(context.selected_objects) > 0
